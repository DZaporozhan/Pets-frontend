...
import { getCurrent } from 'services/api/auth';
// import { addFavorite, deleteFavorite } from 'services/api/notices';
...
export const ModalNotice = ({ id, handleModalToggle, favorite }) => {
  const { data: notices, isSuccess } = getOneNotice(id);
  const { data: currentUser } = getCurrent();
  const { data: owner = [] } = getUserNotices(id);
  ...
  const [deleteNotices] = removeNotice();
